...
| 平台 | 当前状态 |
| --- | --- |
| macOS 14.4 或更高版本 | [从 GitHub Releases 下载经过签名和公证的 `.pkg` 安装包](https://github.com/MisakiHCL/roammand/releases/latest/download/Roammand.pkg)。 |
| iPhone 和 iPad | **尚未上线。** 1.0.0 版本正在等待 App Store 发布。[查看即将上线的 App Store 页面](https://apps.apple.com/app/id6792014935)。 |

上面的 App Store 链接用于标识即将发布的正式版本，目前还不是公众下载入口。
TestFlight 仍仅供受邀测试者使用；macOS 历史版本及校验信息见
[GitHub Releases](https://github.com/MisakiHCL/roammand/releases)。

## 如何使用

1. 在 Mac 上安装 Roammand，然后打开“**此电脑**”。
2. 按照 App 提示完成“屏幕录制”和“辅助功能”设置。两项权限就绪前，Host
   会保持不可用。
3. 创建手机二维码邀请，在 iPhone 或 iPad 上扫描，核对四个验证词，然后在
   Mac 上批准名称正确的 Controller。
...
Mac 用户核对并批准名称正确的 Controller 后，配对才会创建保存在 Host 本机的
...
Roammand 提供固定版本的 signaling 与 STUN-only coturn Docker Compose 部署，以非 root 服务、TLS 文件 secrets、健康检查、单一 UDP STUN 端口和有界日志运行。参见 [Docker Compose 自托管](docs/self-hosting/docker-compose.zh-CN.md)。
